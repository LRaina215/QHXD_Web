...
JSON_SCHEMA = {
    "intent": "go_to_waypoint | start_patrol | pause_task | resume_task | return_home | query_self_identity | query_capability | query_safety_rule | query_robot_status | query_task_status | query_battery | query_emergency_stop | query_perception_status | query_weather | query_environment | speak_last_result | unknown",
    ...
    "reason": "一句简短中文原因",
    "reply_text": "查询类可给一句简短中文回复；任务类可给确认提示；无法确定时为 null",
...
def build_prompts(recognized_text: str) -> tuple[str, str]:
    ...
    user_payload = {
        ...
        "safety_rules": [
            ...
            "查询身份、能力、安全规则、状态、任务、电量、急停、视觉、天气时，只输出对应 query intent，不要编造外部数据。",
        ],
    }
