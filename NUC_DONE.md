...
- RK3588 后端接口可访问：
  - `GET /docs` 正常
  - `GET /openapi.json` 正常
  - `POST /api/system/mode/switch` 正常，已验证可切换到 `real` 模式

## 当前联调结论

应用层接口本身已经打通。

本次联调使用有线网口固定 IPv4 方案：

- NUC 有线网口 `enp89s0`: `192.168.10.3/24`
...
- 给 RK3588 有线口配置固定 IPv4
- 给 NUC 有线口配置固定 IPv4
- 两端放在单独的直连或同交换机网段
- `configs/default_config.json` 中的 `rk3588.base_url` 最终应改为该有线 IP
